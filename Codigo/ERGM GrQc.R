...
base <- read.delim("CA-GrQc.txt", comment.char="#")
...
# Guardamos el modelo
save(modelo_simple, file = "ERGM_B3.RData")
...
write.csv(data.frame(valor = predicted_prob), file = "predict_GrQc.csv", row.names = FALSE)
write.csv(data.frame(valor = observed), file = "observed_GrQc.csv", row.names = FALSE)
